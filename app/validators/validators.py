...
class Registration(BaseModel):
    ...
    height: Optional[float] = Field(None, description="рост, от 130 до 210")
    ...
    def validate_goal(cls, v):
        if v is not None and (v not in ("Набрать", "Похудеть")):
    ...
    def validate_kbju_setting(cls, v):
        if v is not None and (v not in ("Самостоятельно", "Автоматически")):
            raise ValueError("Нажми на кнопки на месте твоей клавиатуры для выбора.")
        return v
    ...
    def validate_gender(cls, v):
        if v is not None and (v not in ("Мужской", "Женский")):
            raise ValueError("Нажми на кнопки на месте твоей клавиатуры для выбора.")
        return v
    ...
    def validate_activity_level(cls, v):
        if v is not None and (v not in ("Низкая", "Умеренная", "Высокая")):
            raise ValueError("Нажми на кнопки на месте твоей клавиатуры для выбора.")
        return v
...
class ValuesKBJU(BaseModel):
    ...
    KBJU: str = Field(description="значения КБЖУ, передаются строкой")
    ...
    def validate_values(self):
        try:
            ...
            raise AttributeError("Кажется, ты не ввел текст, попробуй еще раз.")
